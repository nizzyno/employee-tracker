...
const {getEmployees, getManagers, addEmployee, updateRole} = require('./employees-fetch');
const {getDepartments, addDepartment} = require('./departments-fetch');
const {getRoles, addRole} = require('./roles-fetch');
var employeesTable = [];
var roles = [];
var rolesOnly = [];
var departments = [];
...
class App {
    constructor() {
        ...
        this.mainMenuPrompt = [
            {
                ...
                name: "toDo",
                choices: ["View All Employees", "View All Departments", "View All Roles", "Create New Department", "Create a New Role", "Add New Employee", "Update Employee Role", "Exit"],
            }
        ];
        ...
        this.addDepartmentPrompt = [
            {
                type: "input",
                message: "Creating a new Department!. Please enter the name of the department you wish to create:",
                name: "name",
                validate: nameInput => {
                    if (nameInput) {
                        return true;
        ...
        this.addRolePrompt = [
            {
                type: "input",
                message: "Creating a new Role!. Please enter the title of the role you wish to create?",
                name: "title",
                validate: nameInput => {
                    if (nameInput) {
                        return true;
            ...
            {
                type: "list",
                message: ({title}) => `Please select a department to assign to ${title} role:`,
                name: "department_id",
        ...
        this.addEmployeeprompt = [
            {
                type: "input",
                message: "Adding a new Employee!. Please enter the first name of the new Employee:",
                name: "first_name",
                validate: nameInput => {
                    if (nameInput) {
                        return true;
        ...
        this.updatingRolePrompt = [
            {
                type: "list",
                message: "Please select the employee you wish to update his or her role:",
                name: "id",
                choices: managersOnly,
            },
            {
    ...
    createDepartment() {
        ...
        inquirer.prompt(this.addDepartmentPrompt).then(async data => {
    ...
    createNewRole(departmentsOnly) {
        ...
        inquirer.prompt(this.addRolePrompt).then(async data => {
    ...
    addNewEmployee(rolesOnly, managersOnly) {
        ...
        inquirer.prompt(this.addEmployeeprompt).then(async data => {
    ...
    mainMenu() {
        // promp user on what it wants to do
        inquirer.prompt(this.mainMenuPrompt).then(async data => {
    ...
    exitMessage () {
        console.log(`
        ************************************************************
        ******** THANK YOU FOR USING EMPLOYEE MANAGER V1.0 *********
        ******************** HAVE A GREAT DAY! *********************
        ************************************************************        
        `);
        return
